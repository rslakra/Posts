...
class HTTPStatus(AutoUpperCase):
    ...
    200	OK - The HTTP 200 OK successful response status code indicates that a request has succeeded. A 200 OK response is cacheable by default.
    ...
    415	Unsupported Media Type - The server does not support the request data format.
    422	Unprocessable Entity - The request data was properly formatted but contained invalid or missing data.
    429 Too Many Requests - The client has sent too many requests in a given amount of time. This mechanism of asking the client to slow down the rate of requests is commonly called "rate limiting".
    500	Internal Server Error - The server encountered an unexpected condition that prevented it from fulfilling the request.
    501 Not Implemented - The server does not support the functionality required to fulfill the request.
    503 Service Unavailable - The server is not ready to handle the request.
    504 Gateway Timeout - The server, while acting as a gateway or proxy, did not get a response in time from the upstream server in order to complete the request.
    """
    
    OK = (200, 'OK')  # The HTTP 200 OK successful response
    CREATED = (201, 'Created')  # A new resource was created.
    ACCEPTED = (202, 'Accepted')  # The request was received, but no modification has been made yet.
    ...
    UNAUTHORIZED = (401, 'Unauthorized')  # The client is not authorized to perform the requested action.
    ...
    UNSUPPORTED_MEDIA_TYPE = (415, 'Unsupported Media Type')  # The server does not support the request data format.
    INVALID_DATA = (422, 'Unprocessable Entity')
    TOO_MANY_REQUESTS = (429, 'Too Many Requests')  # the client has sent too many requests in a given amount of time.
    INTERNAL_SERVER_ERROR = (500, 'Internal Server Error')
    NOT_IMPLEMENTED = (501, 'Not Implemented')
    SERVICE_UNAVAILABLE = (503, 'Service Unavailable')  # The server is not ready to handle the request.
